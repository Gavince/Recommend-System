...
class PLE(nn.Module):
    ...
    def __init__(self, user_feature_dict, item_feature_dict, emb_dim=128, hidden_out_size=64, num_experts=8,
                 selectors=2, num_tasks=2, num_levels=2):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_levels):
            if i == self.num_levels - 1:
                ...
                _, task1_o, task2_o = self.gate2(ple_inputs[0], ple_inputs[1], ple_inputs[2])
            else:
                ple_outputs = self.gate1(ple_inputs[0], ple_inputs[1], ple_inputs[2])
